...
def render_photography_pricing() -> None:
    ...
    _render_editor_fragment(selected_client, selected_internal, quote_metadata)


@st.fragment
def _render_editor_fragment(selected_client: Any, selected_internal: Any, quote_metadata: Any) -> None:
    # Setup changes trigger full runs, refreshing these arguments. Keep every
    # pricing-dependent display and all editor containers inside this boundary.
    ...
    page1_header_payload = _build_page1_header_payload(
        quote_metadata, selected_client, selected_internal_payload,
    )
    pdf_inputs = {
        "quote": quote_payload,
        "header": page1_header_payload,
        "comments": st.session_state.get("photo_pricing_page1_comments_payload"),
    }
    _invalidate_stale_generated_pdf(st.session_state, pdf_inputs)
    ...
    if st.button("Generate PDF", key="photo_pricing_generate_pdf", type="primary", width="stretch"):
        ...
        if errors:
            ...
            st.session_state["photo_pricing_generated_pdf_inputs"] = deepcopy(pdf_inputs)
...
def _invalidate_stale_generated_pdf(state: Any, current_inputs: dict[str, Any]) -> None:
    """Compare only PDF dependencies; unrelated reruns preserve generated bytes."""
    if state.get("photo_pricing_generated_pdf_inputs") != current_inputs:
        state.pop("photo_pricing_generated_pdf", None)
        state.pop("photo_pricing_generated_pdf_inputs", None)
