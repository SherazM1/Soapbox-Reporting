...
def render_drafts_section(
    *,
    ...
    with st.expander("Drafts"):
        ...
        with save_cols[0]:
            if st.button("Save Draft", key="photo_pricing_save_draft", disabled=bool(loaded_draft_id)):
                ...
                try:
                    ...
                    st.session_state["photo_pricing_loaded_draft_id"] = draft.id
                    st.session_state["photo_pricing_loaded_draft_version"] = version.version_number
        ...
        if drafts:
            ...
            versions = _safe_list_versions(str(loaded_draft_id or selected_draft_id))
            if versions:
                ...
                with open_cols[1]:
                    if st.button("Restore Version", key="photo_pricing_restore_version", disabled=not bool(loaded_draft_id)):
                        try:
