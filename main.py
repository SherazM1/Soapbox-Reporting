...
def generate_3p_report(
    data_src=None,                # kept for parity; unused in skeleton
    client_name: str = "",
    report_date: str = "",
    ...
    """
    3P template PDF:
      - Header: logo (top-right), client name, 'Weekly Reporting', date
      - Body: 2×2 teal-outlined cards with inside titles
      - Footer: 'Generated by Soapbox Retail • {today}'
    Returns: PDF bytes.
    """
    # Canvas
    buf = BytesIO()
    ...
    # --- Header ---
    # Logo (auto-resolve ./logo.png if not provided)
    base_path = pathlib.Path(__file__).parent.resolve()
    ...
    # Header text stack (same spacing as 1P)
    c.setFillColor(teal)
    c.setFont("Raleway-Bold", 19)
    ...
    # --- 2×2 Grid ---
    # Why: mirrors 1P panel sizing/positioning for visual parity
    card_w = 3.7 * inch
    card_h = 3.6 * inch
    gap    = 0.15 * inch

    row1_top = h - margin - 1.0 * inch      # aligns with 1P panels
    ...
    def draw_card(x: float, y_top: float, w_: float, h_: float, title: str) -> None:
        # Shell
        c.setFillColor(panel_bg)
        ...
        # Title inside card (match 1P Summary placement)
        title_y_offset = 38
        c.setFont("Raleway-Bold", 22)
        c.setFillColor(navy)
        c.drawCentredString(x + w_ / 2.0, y_top - title_y_offset, title)

        # Placeholder body (why: visual affordance until backend fills content)
        body_top = y_top - title_y_offset - 26
        body_margin = 16
        ph_x = x + body_margin
        ph_y = (y_top - h_) + body_margin
        ph_w = w_ - 2 * body_margin
        ph_h = body_top - ph_y
        if ph_w > 0 and ph_h > 0:
            c.saveState()
            c.setDash(4, 4)
            c.setStrokeColor(colors.Color(teal.red, teal.green, teal.blue, alpha=0.6))
            c.rect(ph_x, ph_y, ph_w, ph_h, stroke=1, fill=0)
            c.restoreState()
            c.setFont("Raleway", 10)
            c.setFillColor(colors.Color(navy.red, navy.green, navy.blue, alpha=0.7))
            c.drawCentredString(ph_x + ph_w / 2.0, ph_y + 6, "Add content later")

    titles = ["Item Sales", "Inventory", "Search Insights", "General Metrics"]
    positions = [
        (x_left,  row1_top),  # top-left
        (x_right, row1_top),  # top-right
        (x_left,  row2_top),  # bottom-left
        (x_right, row2_top),  # bottom-right
    ]
    for (x, y_top), title in zip(positions, titles):
        draw_card(x, y_top, card_w, card_h, title)
