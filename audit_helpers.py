...
def initialize_auditing_session_state(state: dict[str, Any]) -> None:
    defaults = {
        ...
        "audit_competitor_combined_image_orders": {},
        "audit_competitor_multi_image_orders_by_record": {},
        "audit_competitor_assignments": [],
        "audit_competitor_has_multiple_pdps": False,
        "audit_competitor_pdp_group_count": 0,
...
def _parse_dimensions_payload(raw_text: str) -> dict[str, Any]:
    ...
    text = html_lib.unescape(text).replace("\xd7", "x")
    match = re.search(r"(?P<w>\d{2,5})\s*W?\s*[x]\s*(?P<h>\d{2,5})\s*H?", text, flags=re.IGNORECASE)
    if match:
...
def map_sheet_row_to_cached_record(
    *,
    ...
    for i, img in enumerate(extracted_images):
        ...
        image["dimensions_text"] = str(img.get("dimensions_text", "") or "")
